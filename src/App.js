...
function App(props) {
  ...
  function ajtTache(nom, description, localisation, dateHeure) {
    const nouvTache = { id: `todo-${nanoid()}`, nom, description, localisation, dateHeure, completed: false };
  ...
  function modifTache(id, nouvNom, nouvDesc, nouvLoca, nouvDh) {
    const modifListeTache = taches.map((task) => {
      if (id === task.id) {
        return { ...task, nom: nouvNom, description: nouvDesc, localisation: nouvLoca, dateHeure: nouvDh };
      }
      return task;
    });
